...
```bash
xhsops init --tools codex,cursor,vscode
```

或者只初始化特定工具：

```bash
xhsops init --tools cursor
```
...
Cursor 会读取 `.xhsops/` 配置，理解 XHSOps 工作流，帮你完成从选题到发布的全流程。
...
```bash
xhsops quick --idea "你的选题"
xhsops hot --topic "热点话题"
xhsops status --all
```
...
每个工具会共享同一套 `.xhsops/` 配置，数据和状态完全同步。
